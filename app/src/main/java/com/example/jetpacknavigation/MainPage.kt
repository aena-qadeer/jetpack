...
@Composable
fun ParentColumn() {
    var parentText by remember { mutableStateOf("Initial Parent Text") }
    val apiData = listOf("Box 1 Data", "Box 2 Data")
    ...
    Column(
        ...
        Text(text = parentText, fontSize = 22.sp, modifier = Modifier.padding(8.dp))

        apiData.forEach { data ->
            ChildRow(data, onButtonClick = { parentText = "Updated from ${data}" })
        }
    }
}

@Composable
fun ChildRow(data: String, onButtonClick: () -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp)
